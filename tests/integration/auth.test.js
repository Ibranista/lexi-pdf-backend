...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/refresh-tokens', () => {
    ...
    test('should return 401 error if user is not found', async () => {
      ...
      const refreshToken = tokenService.generateToken(userOne.id, expires, tokenTypes.REFRESH);
      await tokenService.saveToken(refreshToken, userOne.id, expires, tokenTypes.REFRESH);
      // deleting the user cascades and removes the token row too, since a Token
      // can't exist without its User in Postgres (unlike the old Mongo setup)
  ...
  describe('POST /v1/auth/send-verification-email', () => {
    ...
    test('should return 204 and send verification email to the user', async () => {
      ...
      const dbVerifyEmailToken = await prisma.token.findFirst({ where: { token: verifyEmailToken, userId: userOne.id } });
  ...
  describe('POST /v1/auth/verify-email', () => {
    test('should return 204 and verify the email', async () => {
      ...
      const dbVerifyEmailToken = await prisma.token.count({
        where: { userId: userOne.id, type: tokenTypes.VERIFY_EMAIL },
      });
